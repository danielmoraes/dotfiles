...
| `meetings`   | `MTGS`    | Local macOS Calendar (or an `.ics` feed)    | `brew install ical-buddy`       |
...
| `icalUrl`               | `$ICAL_URL`         | Optional. Set it to use an `.ics` feed instead of the local calendar. |
| `calendars`             | all                 | Restrict the local read to these calendar names.                      |
...
By default the `meetings` metric reads the **local macOS Calendar** through
[`../../ical/`](../../ical/src/index.ts) (`icalBuddy`). That avoids needing a
private feed URL — Workspace domains commonly disable the per-calendar secret
address — and lets EventKit resolve recurrence and timezones.

Setting `ICAL_URL` switches to the `.ics` path below instead.
